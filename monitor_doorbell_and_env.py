...
def trigger_doorbell():
    ...
    for _ in range(2):
        ...
        #Speaker.tone(1000, 200)      # "Ding" frequency
        time.sleep(0.2)
        M5.Lcd.fillScreen(0xFFFFFF) # White flash
        #Speaker.tone(800, 400)       # "Dong" frequency
        time.sleep(0.4)
    
    # Reset UI after ringing
    draw_ui()


def mqtt_DoorBell_event(msg):
    # This fires when BobsDoorBell/trigger receives a message
...
def connect_mqtt():
    try:
        global mqtt_client  
        mqtt_client = MQTTClient(MQTT_CLIENTID, MQTT_BROKER, port=1883, keepalive=300)
        mqtt_client.connect(clean_session=True)
        
        # Subscribe to environment data
        mqtt_client.subscribe(MQTT_TOPIC_ENV, mqtt_MyTestTopicEnv_event)
        # Subscribe to doorbell trigger
        mqtt_client.subscribe(MQTT_TOPIC_BELL, mqtt_DoorBell_event)
        
        print("Subscribed to Env and Doorbell topics")
